...
function convertPeriodsToDays(periods01, periods02, periodType01, periodType02){
  let periods = [];
...
function periodTotalTax(tax01, tax02, periods01, periods02, periodType01, periodType02, value01, value02){
  let days = convertPeriodsToDays(periods01, periods02, periodType01, periodType02);
  let days1 = days[0];
  let days2 = days[1];
...
function generateResults(){
  ...
  let totalTax = periodTotalTax(tax01, tax02, periods01, periods02, periodType01, periodType02, value01, value02);
  ...
  let taxOverSelicPercentage = taxOverSelic(tax01, tax02, selic, value01, value02)*100;

  errorsList = formValidation(tax01, tax02, periods01, periods02, value01, value02);


  if(errorsList.length <=0){
    results.innerHTML = `A taxa MIX anual é de <span class="results__output--highlight-main">${ponderateTax.toFixed(2)}%</span>, o que representa <span class="results__output--highlight">${taxOverSelicPercentage.toFixed(0)}%</span> da taxa Selic atual.<br> <br> A taxa total do período é de <span class="results__output--highlight"> ${totalTax.toFixed(2)}%.</span> <br> <br> Esta simulação apresenta: <br> <span class="results__output--highlight"> ${valuePercentage01.toFixed(2)}% </span> de Recursos Controlados <br> <span class="results__output--highlight"> ${valuePercentage02.toFixed(2)}% </span> de Recursos Livres <button type="submit" class="form__button" id="redoButton">Refazer</button>`
